angular
    .module('ohapp')
    .controller('pharmacistCtrl', function pharmacistCtrl($scope, $injector, $rootScope, ngDialog) {
        ...
        $scope.updates = {};
        $scope.updates.perfect = {};
        $scope.updates.perfect.hospital = undefined;
        $scope.lead = function (page, order, filter_type, id) {

            //若有数据传递，将传递过来的变量push进对象中。。
            var p = {page: page, order: order, role: 2};
        ...
        $scope.add = function () {
            $scope.updates = {};
            $scope.updates.authentication = 2;
            ngDialog.open({template: 'backend_static/views/popup/p-pharmacist.html', scope: $scope});
            $scope.types = "add";
        };
            ...
            $http
                ...
                .success(function (data) {
                    if (data.status) {
                        ...
                        ngDialog.open({template: 'backend_static/views/popup/p-pharmacist.html', scope: $scope});
                        $scope.hospital = $scope.updates.perfect.hospital;
                        $scope.types = "edit";
                    } else {
                        swal("OMG!", data.message, "error");
                    }
                ...
                if ($scope.types == "edit") {
                    $http
                        .post('/user/update.json', {
                            ...
                            post: $scope.updates.perfect.post,
                            hospital: $scope.hospital,
                            title: $scope.updates.perfect.title,
                            photo: $scope.updates.path,
                            authentication: $scope.updates.authentication,
                            teaching_hospital: $scope.updates.perfect.teaching_hospital,
