angular
    .module('ohapp')
    .controller('groupCtrl', function groupCtrl($scope, $injector, $rootScope, ngDialog) {
        ...
        $scope.add = function () {
            $scope.updates = {};
            ngDialog.open({template: 'backend_static/views/popup/p-groupadd.html', scope: $scope});
            $scope.types = "add";
        };
        $scope.edit = function (id) {
            $http
                .get('/group/info.json', {params: {id: id}})
                .success(function (data) {
                    if (data.status) {
        ...
        $scope.set_jin = function (id) {
            $http
                .post('/group/status.json', {id: id, status: 2})
                .success(function (data) {
                    if (data.status) {
        ...
        $scope.recovery = function (id) {
            $http
                .post('/group/status.json', {id: id, status: 1})
                .success(function (data) {
                    if (data.status) {
