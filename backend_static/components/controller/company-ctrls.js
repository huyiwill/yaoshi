angular
    .module('ohapp')
    .controller('companyCtrl', function companyCtrl($scope, $injector, $rootScope, ngDialog) {
        ...
        $rootScope.root = "/company.html";
        $scope.submitted = false;


        $scope.r_type = ['', '其他', '学校', '事业单位', '私营企业'];
        ...
        $scope.lead = function (page, order, filter_type, id) {
            ...
            p[filter] = id;
            $http
                .get('/company/list.json', {params: p})
...
//          sheng 
        ...
        $scope.add = function () {
            $scope.updates = {};
            ngDialog.open({template: 'backend_static/views/popup/p-company.html', scope: $scope});
            $scope.types = "add";
        };
        $scope.edit = function (id) {
            $http
                .get('/company/info.json', {params: {id: id}})
                .success(function (data) {
                    if (data.status) {
        ...
        $scope.set_jin = function (id) {
            $http
                .post('/company/status.json', {id: id, status: 2})
                .success(function (data) {
                    if (data.status) {
        ...
        $scope.recovery = function (id) {
            $http
                .post('/company/status.json', {id: id, status: 1})
                .success(function (data) {
                    if (data.status) {
