angular
    .module('ohapp')
    .controller('departmentCtrl', function departmentCtrl($scope, $injector, $rootScope, ngDialog) {
        ...
        $rootScope.root="/department.html";
        ...
        $scope.lead = function (page, order, filter_type, id) {
            ...
            $http
                .get('/department/list.json', {params: p})
        ...
        $scope.add = function () {
            $scope.updates = {};
            ngDialog.open({template: 'backend_static/views/popup/p-department.html', scope: $scope});
            $scope.types = "add";
        };
        $scope.edit = function (id) {
            $http
                .get('/department/info.json', {params: {id: id}})
                .success(function (data) {
                    if (data.status) {
        ...
        $scope.set_jin = function (id) {
            $http
                .post('/department/status.json', {id: id, status: 2})
                .success(function (data) {
                    if (data.status) {
        ...
        $scope.recovery = function (id) {
            $http
                .post('/department/status.json', {id: id, status: 1})
                .success(function (data) {
                    if (data.status) {
        ...
        $scope.delete = function (id) {
            swal({
                ...
                if (isConfirm) {
                    $http
                        .post('/department/del.json', {id: id})
                        .success(function (data) {
                            if (data.status) {
