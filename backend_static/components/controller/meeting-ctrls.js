angular
    .module('ohapp')
    .controller('meetingCtrl', function meetingCtrl($scope, $injector, $rootScope, ngDialog, fileUpload) {
        ...
        $scope.set_jin = function (id) {
            $http
                .post('/meeting/status.json', {id: id, status: 2})
                .success(function (data) {
                    if (data.status) {
        ...
        $scope.recovery = function (id) {
            $http
                .post('/meeting/status.json', {id: id, status: 1})
                .success(function (data) {
                    if (data.status) {
        ...
        $scope.release = function (id, type) {
            $http
                .post('/meeting/release.json', {id: id, is_release: type})
                .success(function (data) {
                    if (data.status) {
        ...
        $scope.sign_up = function (id, type) {
            $http
                .post('/meeting/enroll.json', {id: id, enroll_state: type})
                .success(function (data) {
                    if (data.status) {
        ...
        $scope.close_meet = function (id, type) {
            $http
                .post('/meeting/state.json', {id: id, state: type})
                .success(function (data) {
                    if (data.status) {
        ...
        $scope.shenhe = function (id) {
            swal({
                ...
                if (isConfirm) {
                    $http
                        .post('/meeting/examine.json', {id: id, examine_type: 2})
                        .success(function (data) {
                            if (data.status) {
                                swal("干得漂亮！", "审核成功！", "success");
                                $scope.lead('', '', $scope.soso_end_type, $scope.soso_end_text);
                            } else {
                                swal("OMG!", data.message, "error");
                            }
                    ...
                    $http
                        .post('/meeting/examine.json', {id: id, examine_type: 3})
                        .success(function (data) {
                            if (data.status) {
                                swal("干得漂亮！", "审核成功！", "success");
                                $scope.lead('', '', $scope.soso_end_type, $scope.soso_end_text);
