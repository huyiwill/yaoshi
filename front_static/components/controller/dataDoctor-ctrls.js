angular
    .module( 'ohapp' )
    .controller( 'dataDoctorCtrl', function dataDoctorCtrl( $scope, $injector, $rootScope) {
        ...
        $scope.isOtner = function (i) {
            ...
            if($scope.otnerhostipal==2){
                ...
                $scope.teaching_department = undefined;
            }
        };
        ...
        $scope.send_end = function(){
            $http
                .post('/user/perfect.json',{id:$scope.id,role:3,name:$scope.name,birthday:$(".birthday").val(),hospital:$scope.hostipal,title:$scope.title,teaching_hospital:$scope.teaching_hospital,head:$scope.head,photo:$scope.path,department:$scope.department,teaching_department:$scope.teaching_department})
                .success(function (data) {
                    if(data.status){
