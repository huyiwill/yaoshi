angular
    .module( 'ohapp' )
    .controller( 'testErrorsCtrl', function testErrorsCtrl( $scope, $injector, $rootScope,$stateParams) {
        ...
        $scope.isHave = false;
        for(var i in $session.get("userMsg_error")){
            $scope.isHave=true;
        }
        if($scope.isHave){
            $scope.hosity_popup = 1;
            $scope.userMsg = $session.get("userMsg_error");
        }

        //获取题目
        $http
            .post('/purpose/list/two.json')
            .success(function (data) {
                if (data.status) {
        ...
        $scope.click_end = function (key, id, s) {
            if ($scope.old_id == id) {
                ...
                if ($scope.exercise_text.right_key == key) {
                    ...
                    $http
                        .post('/purpose/cancel/two.json', {subject_id: $scope.exercise_text.id})
                        .success(function (data) {
                ...
                if($scope.ex_num == $scope.exercise_all.length){
                    ...
                    $session.purge('userMsg_error');
                }else{
                    //记录用户的答题相关信息
                    $session.set('userMsg_error',{chose_dan:$scope.is_dan,num:Number($scope.ex_num)+1,answer_right:$scope.answer_right,answer_error:$scope.answer_error,correct:$scope.correct});
                    $session.save();
                }
            }
        }
