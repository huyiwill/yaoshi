...
class MeetingController extends Controller
{
    ...
    public function actionData(Request $request, Response $response)
    {
        ...
        $meeting_id = $this->db->meeting()->where(['id' => $post['id'], 'status'=>[1,2]])->fetch('id');
        if (empty($meeting_id)) {
    ...
    public function actionAdd(Request $request, Response $response)
    {
        ...
        $meeting_id = $this->db->meeting()->where(['name' => $post['name'], 'status'=>[1,2]])->fetch('id');
        if ($meeting_id) {
        ...
        $insert_row = [
            ...
            'examine_type' => self::EXAMINE_TYPE_TWO,
            '`from`' => 2,
            'create_time' => time()
        ];
    ...
    public function actionDetailAdd(Request $request, Response $response)
    {
        ...
        $meeting_details_id = $this->db->meeting_details()->where(['meeting_id' => $post['meeting_id'], 'sort' => $post['sort'], 'status'=>[1,2]])->fetch('id');
        ...
        $meeting_info = $this->db->meeting()->where(['id' => $post['meeting_id'], 'status'=>[1,2]])->fetch();
        if (!$meeting_info) {
            return $response->withHeader('Content-type', 'application/json')->write(json_encode(['status'=>false,'message'=>'会议不存在']));
        }
    ...
    public function actionDetailInfo(Request $request, Response $response)
    {
        ...
        $meeting_info = $this->db->meeting()->where(['id' => $get['meeting_id'], 'status'=>[1,2]])->fetch();
        if (!$meeting_info) {
            return $response->withHeader('Content-type', 'application/json')->write(json_encode(['status'=>false,'message'=>'会议不存在']));
        }
    ...
    public function actionDetailUpdate(Request $request, Response $response)
    {
        ...
        //detail info
        $meeting_details_info = $this->db->meeting_details()->where(['id' => $post['id'], 'status'=>[1,2]])->fetch();
        ...
        if ($meeting_details_info['type'] == self::DETAIL_TYPE_ONE) {
            ...
            $this->db->transaction = 'BEGIN';
            $meeting_schedule_delete = $this->db->meeting_schedule()->where(['meeting_id'=>$post['meeting_id']])->update(['status'=>3]);
            $schedule_result = $this->_insert_schedule($post);
            $update_row = [
                'sort' => $post['sort'],
                'type' => $post['type'],
            ...
            // transaction
            $this->db->transaction = 'BEGIN';
            $meeting_guest_delete = $this->db->meeting_guest()->where(['meeting_id'=>$post['meeting_id']])->update(['status'=>3]);
            $guest_result = $this->_insert_guest($post);
            $update_row = [
                'sort' => $post['sort'],
                'type' => $post['type'],
    ...
    /* 审核 */
    public function actionExamine(Request $request, Response $response)
    {
        $post = $request->getParsedBody();
        //form control empty
        if (!$this->formControlEmpty(['id'], $post)) {
    ...
    public function actionUpdate(Request $request, Response $response)
    {
        ...
        $cache_user_info = $this->getLogin('_ys_backend_login', $request);
        $meeting_info = $this->db->meeting()->where(['id' => $post['id'], 'status'=>[1,2]])->fetch();
        if (!$meeting_info) {
            return $response->withHeader('Content-type', 'application/json')->write(json_encode(['status'=>false,'message'=>'会议不存在']));
        }
    ...
    public function actionList(Request $request, Response $response)
    {
        ...
        if (!empty($data)) {
            array_walk($data, function (&$item) {
                //creator
                if ($item['uid_admin'] > 0) {
                    $item['creator'] = $this->db->user_global()->where(['id'=>$item['uid_admin']])->fetch('mobile');
                } else if ($item['admin_id'] > 0) {
                    $item['creator'] = $this->db->admin()->where(['id'=>$item['admin_id']])->fetch('account');
                } else {
                    $item['creator'] = '错误数据';
                }
            });
    ...
    public function actionInfo(Request $request, Response $response)
    {
        ...
        $meeting_info = $this->db->meeting()->where(['id' => $get['id'], 'status'=>[1,2]])->fetch();
        if ($meeting_info) {
            $meeting_info = iterator_to_array($meeting_info->getIterator());
            //detail
